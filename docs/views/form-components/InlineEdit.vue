...
<template>
  <main>
    ...
    <p>
      Inline edit can be wrapped around a input field. It starts in a read-only but can be activated when the user
      clicks or taps the field. Once activated the input appears and can be edited and saved.
    </p>
    ...
    <h4>With autosave</h4>
    <p>
      Adding the attribute <code>data-autosave</code> to the component will trigger an extra event (autosave) when the
      user has focused away from the input field.
    </p>
    ...
    <ul>
      ...
      <li>
        The input should have a ‘Save’ and ‘Cancel’ button overlaying the page content located to the right and below
        the input.
      </li>
      <li>
        (Option 1) User must manually save field: the user can change the value in the field then click or press ‘Save’
        or ‘Enter’ to change the value or click or press cancel to return the field to the previously entered value. If
        they click onto another element on the page the field will go into the unsaved state where it remembers the
        changes. When the user clicks into the field it will show the field with the changes but the user must manually
        save to change the field value permanently.
      </li>
      <li>
        (Option 2) Field saves when the user clicks away - the user can change the value in the field then either click
        or press ’Save’, ‘Enter’, or click another element on the page to save the value permanently. To cancel the
        change the user would need to click the cancel button.
      </li>
      <li>
        If the value entered in the field is not valid then we can show an optional error message in the browser default
        error popover. The user shouldn’t be able to save the field until the error has been resolved.
      </li>
    </ul>
    ...
    <p>
      Text area inline edit is a wrapper around a text area. It starts in a read-only but can be activated when the user
      clicks or taps the field. Once activated the text area appears and can be edited and saved.
    </p>
    ...
    <ul>
      ...
      <li>Clicking the hover state should change the input from read only to editable and show the input.</li>
      <li>
        The input should have a ‘Save’ and ‘Cancel’ button overlayed over the content located to the right and below the
        input.
      </li>
      <li>
        (Option 1) User must manually save field: the user can change the value in the field then click or press ‘Save’
        to change the value or click or press cancel to return the field to the previously entered value. If they click
        onto another element on the page the field will go into the unsaved state where it remembers the changes. When
        the user clicks into the field it will show the field with the changes but the user must manually save to change
        the field value permanently.
      </li>
      <li>
        (Option 2) Field saves when the user clicks away - the user can change the value in the field then either click
        or press ’Save’ or click another element on the page to save the value permanently. To cancel the change the
        user would need to click the cancel button.
      </li>
      <li>
        If the value entered in the field is not valid then we can show an optional error message in the browser default
        error popover.
    ...
    <p>
      Select inline edit is a wrapper around a standard select. It starts in a read-only but can be activated when the
      user clicks or taps the field. Once activated the select appears and can be edited and saved.
    </p>
    ...
    <ul>
      <li>Hovering over the editable area should show the hover state.</li>
      <li>
        Clicking the hover state should change the select from read only to editable and show the select with the
        dropdown open.
      </li>
      <li>
        If the user clicks a value from the dropdown it should change and save the new value and change the inline edit
        to the inactive selected state.
      </li>
      <li>
        If the user clicks away from the select or closes the select without choosing a new value it should return the
        value to the previously selected value and change the inline edit to the inactive state.
      </li>
    </ul>

    <h2 class="mt-5">Multiselect</h2>
    <p>
      Multiselect inline edit is a wrapper around a multiselect. It starts in a read-only but can be activated when the
      user clicks or taps the field. Once activated the multiselect appears and can be edited and saved.
    </p>
    ...
    <ul>
      <li>Hovering over the editable area should show the hover state.</li>
      <li>
        Clicking the hover state should change the multiselect from read only to editable and show the multiselect.
      </li>
      ...
      <li>
        If the value entered in the multiselect is not valid then we can show an optional error message in the browser
        default error popover.
      </li>
    </ul>
    ...
    <p>
      The component works by the web component dispatching a custom event called `inline-edit-save` which can be picked
      up by the pages javaScript so that they can be turned into an api call. The API call will need to dispatch another
      custom event called `inline-edit-saved` after it has saved the value.
    </p>
