...
<template>
  <label for="contact-search" class="mw-100 mb-4 reset-colours"
    ...
    <Search class="search--stylised search--sm" data-icon="fa-solid fa-address-book">
      ...
      <datalist id="contacts">
        <option
          ...
          data-email="auction@iamproperty.com"
          @click="openContact"
        ></option>
        <option
          value="Movebutler compliance team"
          data-phone="0191 234 5678"
          data-email="auction@iamproperty.com"
          @click="openContact"
        ></option>
  ...
  <dialog id="modal-contact" ref="contactModal">
    <Modal>
      ...
      <p>
        <strong>Phone:</strong> <a :href="`tel:${contactPhoneSafe}`" target="_blank">{{ contactPhone }}</a>
      </p>
      <p>
