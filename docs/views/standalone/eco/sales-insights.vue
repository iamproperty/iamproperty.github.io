<script setup lang="ts">
  ...
  const saveCompetitors = (event): void => {
    ...
    checkAccount.value.selectedCompetitors = Array.from(
      selectedCompetitorsFieldset.querySelectorAll('input:checked')
    ).map((x) => x.value);
    // #endregion
    charts.value = [];
  ...
  const searchOutcodes = async (event): void => {
    ...
    if (!event.srcElement.shadowRoot) return false;
  ...
  <div class="admin-panel bg-white">
    ...
    <div class="d-flex">
      ...
      <button
        id="customise-btn"
        ref="customiseBtn"
        class="btn btn-action fa-cog"
        type="button"
        command="show-modal"
        commandfor="competitor-list"
        :disabled="!checkAccount.connected || !checkAccount.agreedTerms || !checkAccount.outcodes"
      >
        Customise
      </button>
    </div>
    ...
    <div v-if="checkAccount.connected && !checkAccount.agreedTerms" id="view-terms" class="bg-primary">
      ...
      <div class="text-center pt-3">
        ...
        <span class="h4 px-5 pb-3"
          >Please agree to terms of use to <span class="text-info">unlock</span> this data</span
        >
        <button class="btn btn-primary colour-warning" command="show-modal" commandfor="agree-terms-modal">
          View terms
        </button>
      </div>
    </div>
    ...
    <p v-if="checkAccount.connected && checkAccount.agreedTerms && checkAccount.outcodes" id="selected-outcodes">
      Outcode area: <span v-for="outcode in checkAccount.outcodes" :key="outcode">{{ outcode }}</span>
    </p>

    <span v-for="competitor in checkAccount.selectedCompetitors" :key="competitor.name">{{ competitor.name }}</span>

    <Tabs
      v-if="checkAccount.connected && checkAccount.agreedTerms && checkAccount.outcodes && charts"
      class="tabs--toggle-tags"
    >
    ...
    <a
      v-if="checkAccount.connected && checkAccount.agreedTerms && checkAccount.outcodes"
      href="/"
      target="_blank"
      class="btn btn-primary"
      >Compare against my branch</a
    >

    <Notification
      v-if="checkAccount.connected && checkAccount.agreedTerms && checkAccount.outcodes"
      class="colour-info mb-1"
    >
    ...
    <Modal class="modal--lg">
      <dialog id="competitor-list" aria-labelledby="competitor-list-title">
        ...
        <Multiselect
          data-label="Search outcodes"
          data-tooltip="Tooltip text"
          @change="onOutcodeChange"
          @input="searchOutcodes"
          @clear="clearEvent"
        >
        ...
        <div class="btn__group text-end">
          ...
          <button class="btn btn-primary" command="close" commandfor="competitor-list" @click="saveCompetitors()">
            Update competitors
          </button>
        </div>
      </dialog>
    ...
    <Modal data-type="transactional" data-agreed-text="Agree terms" @agreed="agreeTerms()">
      <dialog id="agree-terms-modal" aria-labelledby="agree-terms-title">
        ...
        <p>
          iamproperty provide the sales data on an "as is" basis and makes no representations or warranties, express or
          implied, as to the accuracy, completeness, or reliability of the data. The Agent acknowledges that the data is
          supplied for their internal analysis and may not be used for general marketing purposes or shared with anyone
          outside of the Agent's employment. iamproperty shall not be liable for any claims, damages, losses, or
          expenses arising from the use or reliance upon the data, and the Agent agrees to indemnify and hold harmless
          iamproperty from any such claims.
        </p>
      </dialog>
    </Modal>
