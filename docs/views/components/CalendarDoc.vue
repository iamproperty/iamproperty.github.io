<script setup lang="ts">
  ...
  const saveEvent = (): void => {
    ...
    (document.querySelector('iam-calendar') as HTMLDialogElement).insertAdjacentHTML('beforeend',`
...
<template>
  <TrackEvents
    ...
    :events="['right-click','open-filters','change-view','change-date','change-date-today','change-date-previous','change-date-next','open-settings','close-settings','save-settings']"
  ></TrackEvents>
...
<form id="calendar-filters" @change="setAttributes">
      ...
      <fieldset class="calendar-option">
        ...
        <select name="event-colour" class="btn btn-action event-colours">
          ...
          <button>
            <span></span><selectedcontent></selectedcontent>
          </button>
    ...
    <Calendar class="md-col-start-4 calendar--multi" data-calendars="John Smith, Inspections" data-views="day,week,month" data-view="month" data-filter @click="editEvent" @right-click="openMenu">
      ...
      <button datetime="2025-07-21" data-days="4" data-event-type="Inspection" data-calendar="John Smith" class="cancelled">
        Appraisal: 1 Lake Avenue, Newcastle upon Tyne, Tyne and Wear, NE13 7FH, UK
        <span class="tooltip__content">
          Appraisal: 1 Lake Avenue, Newcastle upon Tyne, Tyne and Wear, NE13 7FH, UK<br/>
          Calendar type: Sales<br/>
