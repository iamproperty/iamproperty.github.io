<script setup>
  ...
  const percent = ref(0);
  const arrSaved = ref(
    localStorage.getItem('checkedStories') ? JSON.parse(localStorage.getItem('checkedStories')) : []
  ); // Load rom device if present

  const filename = ref(
    document.querySelector('#filename') ? document.querySelector('#filename').value() : 'application'
  );
  ...
  const launch = ($event) => {
    ...
    Array.from(document.querySelectorAll('tr:has(input[type="checkbox"]:checked) td[data-label="Story"]')).forEach(
      (td) => {
        if (!isSaved(td.innerHTML)) arrSaved.value.push(td.innerHTML);
      }
    );

    Array.from(
      document.querySelectorAll('tr:has(input[type="checkbox"]:not(:checked)) td[data-label="Story"]')
    ).forEach((td) => {
...
<template>
  <main class="integration-launch-list">
    <h1>Launch list</h1>
    <p class="lead">
      Our non-functional requirements can be used to test the readiness of an application. Designed to measure the
      usability and inclusiviness of web pages.
    </p>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="nfr in group" v-bind:key="nfr.story">
              <td :data-required="`${nfr.required}`">
                {{ nfr.story }}{{ nfr.required == 'true' ? `` : ` (Optional)` }}
              </td>
              ...
              <td class="text-end">
                <label class="me-0"
                  ...
                  ><input
                    type="checkbox"
                    :name="`check-${i++}`"
                    :checked="isSaved(nfr.story)"
                    @change="($event) => launch($event)"
                /></label>
              </td>
            </tr>
          </tbody>
    ...
    <div class="text-end mb-5">
      <button class="btn btn-primary" @click="($event) => submit($event)">Save to CSV</button>
    </div>
