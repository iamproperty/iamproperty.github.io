<script setup>
  ...
  const showExtend =
    slots['dispatched-events'] || slots['event-listeners'] || slots['watched-attrs'] || slots['observers']
      ? true
      : false;
  const showTest = true;
...
<template>
  ...
  <Tabs class="container">
    <details v-if="showInstall">
      ...
      <pre
        class="mb-5"
        v-if="slots['web-component']"
      ><code>import('../node_modules/@iamproperty/components/assets/js/components/{{ props.component }}/{{ props.component }}.component.min').then(module => { // Might need to update the path
  ...
  console.log(err.message);
    ...
    <details v-if="showStyle">
      ...
      <small v-if="slots['vars']" class="d-block pb-5"
        >*Parts can be targeted in the pages CSS be using a selector like `iam-card::part(body)` with the part name
        being passed into the part function.</small
      >
    ...
    <details v-if="showExtend">
      ...
      <small v-if="slots['dispatched-events']" class="d-block pb-5"
        >For the page to extend this event it needs to create an event listener on the component, see below:
        <br /><br /><code
          >cardComponent.addEventListener('dispatched-event-name', function(event){ ...do something });</code
        ></small
      >
      ...
      <small v-if="slots['event-listeners']" class="d-block pb-5"
        >Events can also be dispatched onto the component and listened for using an event listener created inside of the
        component JavaScript, see below: <br /><br /><code
          >cardComponent.addEventListener('event-name', function(event){ ...do something });</code
        ></small
      >
