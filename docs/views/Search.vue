    ...
    <div class="container">
      ...
      <div v-for="item in results">
          <a :href="`${item.path}?searchterm=${searchTerm.replace(' ','_')}`">
            <Card>
              {{ item.name }}
              <span v-if="item.searchterms"><i>{{item.searchterms}}</i></span>
            </Card>
          </a>
      </div>
      ...
      <hr/>
      <p class="note mb-5 mt-3"><strong>Note:</strong> The search functionality works from the page titles and pre-defined search terms. If you feel that any search terms are missing please provide feedback to one of the Front End Guild members.</p>
    </div>
...
routes.forEach((route) => {
  ...
  if(name && name.toLowerCase().trim().includes(searchTerm) && name != "Search" && name != "Home" && name != "NotFound")
    results.push(route);
  else if(route.searchterms && route.searchterms.includes(searchTerm))
    results.push(route);
