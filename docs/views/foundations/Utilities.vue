    ...
    <div class="container">
      ...
      <ul class="list-unstyled">
        <li :class="'pb-4 '+value.class" v-for="(value, name) in utClasses" :key="name">
          <h2 :class="'h5 '+value.class">{{name}} <span class="strong text-success" v-if="value.added">Added</span></h2>
...
Object.keys(shared.themeColours).forEach(function (element){
  utClasses['border-'+element.toLowerCase()] = {'name': 'border-'+element.toLowerCase(), 'value':'Removed', 'class': 'text-danger'}
});
let floatClass = ['float-end','float-md-end','float-md-none','float-md-start','float-none','float-sm-end','float-sm-none','float-sm-start','float-start']
