<script setup lang="ts">
  ...
  const insight = ref(props.insight || '');
  ...
  const defaultGroupOrder = ['Win', 'Grow'];
  //const dashboards = ref<any[]>(props.dashboards || []);

  const createGroups = (dashboards) => {
    return [...new Set<string>(dashboards.map((dashboard) => dashboard.group))]
      .sort((a, b) => {
        const aIndex = defaultGroupOrder.indexOf(a);
        const bIndex = defaultGroupOrder.indexOf(b);
        return (
          (aIndex === -1 ? defaultGroupOrder.length : aIndex) - (bIndex === -1 ? defaultGroupOrder.length : bIndex)
        );
      })
      .map((name) => ({
        name,
        children: dashboards.filter((dashboard: any) => dashboard.group === name),
      }));
  };

  const groups = ref<{ name: string; children: [] }[]>(props.dashboards ? createGroups(props.dashboards) : []);
  ...
  watch(
    () => props.dashboards,
    (newId, oldId) => {
      groups.value = newId ? createGroups(newId) : [];
    }
  );
    ...
    elements.forEach((element) => {
      ...
      if (!isEnabled && element.getAttribute('data-disabled')) {
        ...
        element.addEventListener('click', (event) => {
          ...
          let a = document.createElement('a');
          //a.target= '_blank';
          a.href = `/prototypes/one-platform-hub/${element.getAttribute('data-slug')}`;
          a.click();
        });
      }
    });
    ...
    <div class="text-center">
      <div class="brand brand--insights p-1">
        <svg viewBox="0 0 600 130" preserveAspectRatio="xMinYMid meet">
          ...
          />
          <path
            d="M141.74,61.39v33.3c0,1.82-1.03,2.85-2.85,2.85h-11.4c-1.82,0-2.85-1.03-2.85-2.85v-31.82c0-5.7-3.31-8.21-7.64-8.21-5.13,0-8.32,2.85-8.32,9.58v30.45c0,1.82-1.03,2.85-2.85,2.85h-11.4c-1.82,0-2.85-1.03-2.85-2.85v-51.32c0-1.82,1.03-2.85,2.85-2.85h11.4c1.82,0,2.85,1.03,2.85,2.85v2.39c2.62-3.88,7.53-6.84,14.83-6.84,10.61,0,18.25,8.21,18.25,22.47Z"
          />

          <path
            d="M236.15,94.27v-51.21c0-1.82,1.02-2.85,2.83-2.85h11.32c1.81,0,2.83,1.02,2.83,2.85v51.22c0,1.82-1.02,2.85-2.83,2.85h-11.32c-1.81,0-2.83-1.02-2.83-2.85h0Z"
          />
          <path
            d="M236.15,30.54v-10.24c0-1.82,1.02-2.85,2.83-2.85h11.32c1.81,0,2.83,1.02,2.83,2.85v10.24c0,1.82-1.02,2.85-2.83,2.85h-11.32c-1.81,0-2.83-1.02-2.83-2.85Z"
          />
          <path
          ...
          />
          <path
            d="M363.26,80.05c0,13.09-11.09,18.67-23.54,18.67-11.55,0-21.96-5.24-23.32-17.3-.23-1.94.9-2.96,2.72-2.96h11.09c1.7,0,2.27.91,3.06,2.39.9,1.94,3.06,3.07,6.45,3.07,4.75,0,6.57-1.59,6.57-3.87,0-8.2-29.55-.68-29.55-22.76,0-12.86,11.1-18.67,22.98-18.67,10.53,0,20.83,5.24,22.19,17.3.23,1.93-.9,2.96-2.72,2.96h-11.09c-1.7,0-2.27-.91-3.06-2.5-.79-1.82-2.72-2.96-5.32-2.96-4.19,0-6,1.59-6,3.87,0,8.42,29.54.57,29.54,22.76Z"
          />
          <path
            d="M369.49,30.54v-10.24c0-1.82,1.02-2.85,2.83-2.85h11.32c1.81,0,2.83,1.02,2.83,2.85v10.24c0,1.82-1.02,2.85-2.83,2.85h-11.32c-1.81,0-2.83-1.02-2.83-2.85Z"
          />
          <path
            d="M369.49,94.27v-51.21c0-1.82,1.02-2.85,2.83-2.85h11.32c1.81,0,2.83,1.02,2.83,2.85v51.22c0,1.82-1.02,2.85-2.83,2.85h-11.32c-1.81,0-2.83-1.02-2.83-2.85h0Z"
          />
          <path
            d="M453.02,43.06v51.22c0,19.12-15.28,27.2-29.09,27.2s-26.83-6.6-28.53-20.26c-.11-1.93,1.02-2.96,2.83-2.96h11.43c1.7,0,2.49.91,3.06,2.5,1.25,3.41,4.98,5.69,11.21,5.69,7.58,0,12.57-4.1,12.57-12.18v-5.12c-3.74,4.44-9.4,7.28-17.32,7.28-14.83,0-27.05-13.09-27.05-28.91s12.23-28.91,27.05-28.91c7.92,0,13.58,2.85,17.32,7.28v-2.85c0-1.82,1.02-2.85,2.83-2.85h10.87c1.81,0,2.83,1.02,2.83,2.85ZM436.5,67.53c0-7.85-5.66-13.2-13.7-13.2s-13.7,5.35-13.7,13.2,5.66,13.2,13.7,13.2,13.7-5.35,13.7-13.2Z"
          />
          <path
          ...
          />
          <path
            d="M548.9,43.06v10.7c0,1.82-1.02,2.85-2.83,2.85h-6.23v37.68c0,1.82-1.02,2.85-2.83,2.85h-11.32c-1.81,0-2.83-1.02-2.83-2.85v-37.68h-6.23c-1.81,0-2.83-1.02-2.83-2.85v-10.7c0-1.82,1.02-2.85,2.83-2.85h6.23v-15.37c0-1.82,1.02-2.85,2.83-2.85h11.32c1.81,0,2.83,1.02,2.83,2.85v15.37h6.23c1.81,0,2.83,1.02,2.83,2.85Z"
          />
          <path
      ...
      <p class="strapline pb-3">
        Turn everyday data into smarter decisions. Win more instructions, retain customers, improve performance and grow
        your business.
      </p>

      <Search id="question-search" class="search--stylised mb-4">
        <input
          id="question"
          type="text"
          name="question"
        ...
        <datalist id="questions" slot="datalist">
          <div v-for="group in groups" :key="group.name" class="optgroup" :value="`${group.name}:`">
            <option
              v-for="dashboard in group.children"
              :key="dashboard.slug"
              :data-product="dashboard.product"
              :data-feature="dashboard.feature"
              :data-disabled="dashboard.linkedDisabled"
...
<style scoped lang="scss">
  .insights-panel {
    ...
    background-image:
      radial-gradient(ellipse 90% 190% at 0% 0%, var(--colour-primary) 15%, transparent 80%),
      radial-gradient(ellipse 90% 190% at 100% 100%, var(--colour-primary) 15%, transparent 80%),
  ...
  #question {
    ...
    background: linear-gradient(180deg, rgb(0 49 60 / 5%), rgb(255 255 255 / 20%));
    backdrop-filter: blur(1.5rem);
    border-radius: 1.5rem;
    box-shadow:
      0 1px 1rem rgba(0, 0, 0, 0.1),
      inset 0 0 1.5rem 0.5rem rgba(0, 49, 60, 0.1);
