...
const createGroups = (dashboards) => {
  return [...new Set<string>(dashboards.map((dashboard) => dashboard.group))].sort(
    (a, b) => {
      const aIndex = defaultGroupOrder.indexOf(a);
      const bIndex = defaultGroupOrder.indexOf(b);
      return (aIndex === -1 ? defaultGroupOrder.length : aIndex)
        - (bIndex === -1 ? defaultGroupOrder.length : bIndex);
    }
  ).map((name) => ({
    name,
  ...
  <div class="insights-panel bg-primary">
    ...
    <div class="text-center">
      ...
      <Search id="question-search" class="search--stylised mb-4">
        <input id="question" type="text" name="question" autocomplete="off" aria-autocomplete="none" list="questions" placeholder="Show me..." :value="search"/>
