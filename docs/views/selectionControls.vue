<template>
  <div class="container">
    <h2>Other selection controls</h2>
    <p class="pb-2">
      Selection controls allow users to complete tasks which involve making choices such as selecting options, switching
      settings on or off, or adding or removing items.
    </p>
    <table>
      <thead>
        <tr>
          <th>Component</th>
          <th>Use case</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><a href="/form/radio">Radio buttons</a></td>
          <td>
            <ul class="mb-0">
              <li>Select a single option from a list</li>
              <li>When the user needs to see all available options in a list</li>
            </ul>
          </td>
        </tr>
        <tr>
          <td><a href="/form/checkbox">Checkboxes</a></td>
          <td>
            <ul class="mb-0">
              <li>Select one or more options from a list</li>
              <li>When the the user can see a selection of options but choose to see all</li>
            </ul>
          </td>
        </tr>
        <tr>
          <td><a href="/form/toggle">Switches</a></td>
          <td>
            <ul class="mb-0">
              <li>Toggle a single item on or off</li>
              <li>Immediately activate or deactivate something</li>
            </ul>
          </td>
        </tr>
      </tbody>
    </table>
  </div>
</template>
