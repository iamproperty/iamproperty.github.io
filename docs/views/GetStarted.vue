<template>
  <main>
    ...
    <p>The assets folder can be used independently from the Vue components so that they can be used with other frameworks. With the compiled files available to use out of the box alongside the source files that can be configured.</p>
...
for (const [key, value] of Object.entries(iamkey)) {
  Vue.component(key, value)
    ...
    <h2 class="pt-3">Contributing to iam key</h2><h3>Create code branch</h3><p>Firstly the clone the <a href="https://github.com/iamproperty/iamproperty.github.io">repo</a> from GitHub and create a new branch from the develop branch using the agreed naming convention (FEG-99/feature/task-title).</p>
...
<p>The second command will run dev but will also watch for sass and typescript files being changed which would then update the CSS and JavaScript files.</p>


<h3>Create Pull request into develop</h3><p>You will not have the ability to commit changes directly to the main or develop branches and a pull request will have to be created. This pull request should follow the format dictated in the .github/pull_request_template.md file. The pull reguest should always be into develop unless you are doing a hotfix in a release branch. At least one approver is needed but for more complex bits of work more can be added. The pull request will run the unit tests and create a test environment in netlify for the reviewer to help complete the review. Once the pull request is closed the netlify development environment will get updated.</p><p><a href="https://iamproperty-dev.netlify.app/">https://iamproperty-dev.netlify.app/</a></p><h2>Create release</h2><p>The creation of an npm package is done through GitHub actions when a GitHub release is created. Releases can be made from any branch but it should come from the master branch. <a href="https://www.npmjs.com/package/@iamproperty/components" target="_blank">npm registry package</a>.</p>

    <h2>Update the documentation site</h2>
    <p>To update the documentation site you need to create a pull request from develop or a release branch into master. Once the PR is approved an automatic build and deploy process using GitHub actions will be created.</p>
