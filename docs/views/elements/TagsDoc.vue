<template>
  <main>
    ...
    <h3>Display tags</h3>
    <p>Display tags are used to display quick and useful category labels to the user. They are often used to display categories or active filters. The dismissible property allows for any tag to be actively removed by the user and is often seen in Filtering. If a user can remove or dismiss a badge, the application should provide a way for them to easily add it back. </p>
    ...
    <p>Toggle tags can be turned on and off when selected by the user. Toggle tags are always present on the interface and are often used for filtering data on the page.</p>
    ...
    <p class="mb-5"><strong class="text-danger">Don’t</strong><br/> Use too many colours for tags within one UI. Typically only one color should be used for tags. Mulitples colors should only be used when the meaningful to the user.</p>

    
    <h2 class="">Implementation</h2>
    <p>The tag class can be added to a button or on a label with a checkbox inside. Given that a toggle tag has two states it makes sense to semantically use a checkbox. Display tags however are either visible or not and will require javascript to first display the tag with the tag itself being removed when clicked. So in that case it a button semanticallymakes sense.</p>
