const integrationTests = () => {
  ...
  let test = {};
  const intro =
    'These tests run against mark-up and content meant to test how a component or element has been integrated. Making sure that guidance setout in the design system is being followed.';

  window.integrationTestsIntro = intro;

  console.log(
    '%cVirgin Money Framework Tests',
  ...
  console.log(
    '%c   %c' + 'Successfully implemented',
    'background-color: green; margin-right: 10px',
    'background-color: transparent'
  );
  console.log(
    '%c   %c' + 'NOT implemented correctly',
  ...
  console.log(
    '%c   %c' + 'Warning - No action needed',
    'background-color: orange; margin-right: 10px',
    'background-color: transparent'
  );
  console.log(
    '%c ? %c' + 'Warning - Action needed',
    'background-color: orange; margin-right: 10px',
    'background-color: transparent'
  );
  ...
  Array.from(document.querySelectorAll('.card-deck .card')).forEach((card, index) => {
    if (card.closest('.navbar') == null) {
      ...
      if (btnLink != null && btnLink.textContent.length) test['Passes']++;
      else test['Fails']++;
    }
  });
  ...
  Array.from(document.querySelectorAll('.article-deck')).forEach((deck, index) => {
    ...
    if (container != null && container.matches('[class*="bg-"]')) test['Passes']++;
    else test['Fails']++;
  });
  ...
  Array.from(document.querySelectorAll('.tabs__container--carousel')).forEach((tabs, index) => {
    ...
    if (title != null && strapline != null) test['Passes']++;
    else test['Fails']++;
  });
  ...
  Array.from(document.querySelectorAll('.container.container--split')).forEach((container, index) => {
    if (container.querySelectorAll(':scope > .row > [class*="col"]').length % 2 === 0) test['Passes']++;
    else test['Fails']++;
  });
  test['Total'] = test['Passes'] + test['Fails'];
  errorTable.push(test);
  ...
  Array.from(document.querySelectorAll('.container.container--statement')).forEach((container, index) => {
    if (
      container.querySelectorAll(':scope > *').length == 1 &&
      container.querySelectorAll(':scope > .h6:first-child').length == 1
    )
      test['Passes']++;
    else if (container.querySelectorAll(':scope > *').length == 1) test['Fails']++;
  });
  test['Total'] = test['Passes'] + test['Fails'];
  errorTable.push(test);
  // #endregion
  ...
  Array.from(document.querySelectorAll('.container.container--cta')).forEach((container, index) => {
    if (container.querySelectorAll('h2.h1:first-child').length == 1) test['Passes']++;
    else test['Fails']++;
  });
  test['Total'] = test['Passes'] + test['Fails'];
  errorTable.push(test);
  ...
  Array.from(
    document.querySelectorAll(
      '[class*="p-"],[class*="pt-"],[class*="pb-"],[class*="pl-"],[class*="pr-"],[class*="px-"],[class*="py-"]'
    )
  ...
  Array.from(
    document.querySelectorAll(
      '[class*="m-"],[class*="mt-"],[class*="mb-"],[class*="ml-"],[class*="mr-"],[class*="mx-"],[class*="my-"]'
    )
  ).forEach((element, index) => {
    test['Total']++;
  });
  ...
  console.log(
    'The use of certain utility classes is necessary at times but the over use of these classes will lead to design/layout inconsistencies.'
  );

  errorTableUtility.forEach(function (entry) {
    let color = 'green';

    if (entry['Total'] === 0) color = 'grey';
    else if (entry['Fails'] != 0) color = 'red';
  ...
  Array.from(document.querySelectorAll('h1,h2,h3,h4,h5,h6')).forEach((heading, index) => {
    ...
    if (heading.matches('h2')) level = 2;
    else if (heading.matches('h3')) level = 3;
    else if (heading.matches('h4')) level = 4;
    else if (heading.matches('h5')) level = 5;
    else if (heading.matches('h6')) level = 6;
  ...
  console.groupCollapsed(
    '%c   %c' + 'Headings are in the correct order',
    'background-color: ' + color + '; margin-right: 10px',
    'background-color: transparent'
  );
  ...
  console.groupCollapsed(
    '%c ? %c' + 'Do all of the headings make sense out of context and do they introduce the content that follows?',
    'background-color: ' + 'orange' + '; margin-right: 10px',
    'background-color: transparent'
  );
  console.log(
    'You should be able to read the below table and get an understanding of what the page content consists of. The titles should make sense when read in order and child titles should be relevant to the parent title.'
  );
  console.table(headingTable);
  console.groupEnd();
  // #endregion
  ...
  console.groupCollapsed(
    '%c ? %c' + 'Do all of the links make sense out of context and do they describe the page/section that it links to?',
    'background-color: ' + 'orange' + '; margin-right: 10px',
    'background-color: transparent'
  );
  ...
  console.groupCollapsed(
    '%c ? %c' + 'Do all of the images with an alt tag make sense out of context and describes the image appropriately?',
    'background-color: ' + 'orange' + '; margin-right: 10px',
    'background-color: transparent'
  );
  ...
  console.groupCollapsed(
    '%c ? %c' + 'Can you navigation the page links by tabbing through the page?',
    'background-color: ' + 'orange' + '; margin-right: 10px',
    'background-color: transparent'
  );
  ...
  // #region Lighthouse
  console.groupCollapsed(
    '%c ? %c' + 'Have you ran a lighthouse report on this page',
    'background-color: ' + 'orange' + '; margin-right: 10px',
    'background-color: transparent'
  );
  console.log(
    'If you are in a modern version of chrome you can a lighthouse report by opening the lighthouse window within the devtools. This should appear as a tab link above.'
  );
  ...
  console.log(
    'The below tests work from the current screen size, so if you have your screen at a mobile viewport the scaling and sizes of images will reflect that. It is worth running the image tests in both mobile and desktop views. Simply change the screen size and refresh the page.'
  );
          ...
          if (xhr.status == 200) {
            ...
            if (imgFSize > 1024) {
              ...
              if (imgBPP > 0.5) {
                ...
                bppColor = 'red';
                bppError =
                  "The image has a VERY large \'Bytes per pixel\' score, trying optimising the image using https://tinypng.com/.";
              } else if (imgBPP > 0.2) {
                cssColor = 'orange';
                bppColor = 'orange';
                bppError =
                  "The image has a large \'Bytes per pixel\' score, trying optimising the image using https://tinypng.com/.";
              }
              ...
              if (imgScl > 1.2) {
                ...
                scaleColor = 'red';
                scaleError =
                  'The image size is smaller than the size shown on screen this may cause the image to appear blurry and low quality.';
              } else if (imgScl < 0.5) {
                cssColor = 'red';
                scaleColor = 'red';
                scaleError =
                  "The image size is VERY large compared to the size shown on screen, this means unnecessary kb's are being downloaded. Try resizing the image in photoshop or other image editing software.";
              } else if (imgScl < 0.8) {
                cssColor = 'orange';
                scaleColor = 'orange';
                scaleError =
                  "The image size is large compared to the size shown on screen, this means unnecessary kb's are being downloaded. Try resizing the image in photoshop or other image editing software.";
              }
              ...
              console.log(
                ...
                'background: url(' +
                  ...
                  '); border: 3px solid ' +
                  cssColor +
                  '; line-height: 30px; width: 30px; background-size: cover;',
                'background-color: transparent'
              );
              ...
              if (bppError != '') {
                console.log(
                  '%c   %c' + bppError,
                  'background-color: ' + bppColor + '; margin-right: 10px',
                  'background-color: transparent'
                );
              }
              if (scaleError != '') {
                console.log(
                  '%c   %c' + scaleError,
                  'background-color: ' + scaleColor + '; margin-right: 10px',
                  'background-color: transparent'
                );
              }
