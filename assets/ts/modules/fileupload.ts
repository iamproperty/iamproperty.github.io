function fileupload(fileupload: Element, wrapper: Element): void {
  ...
  const checkFileExt = function (filename): boolean {
    ...
    if (acceptedTypes.includes(nameExtension)) return true;
  ...
  wrapper.addEventListener('click', (event) => {
    if (event && event.target instanceof HTMLElement && event.target.closest('.files button')) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (file.name != button.getAttribute('data-file')) dt.items.add(file); // here you exclude the file. thus removing it.
      }
