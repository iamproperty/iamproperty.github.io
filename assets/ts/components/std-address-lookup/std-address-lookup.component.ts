...
class iamSTDAddressLookup extends HTMLElement {
  ...
  connectedCallback(): void {
    ...
    if (this.hasAttribute('data-address-unknown')) {
      const addressUnknownInput = this.querySelector(
        `[name="${this.hasAttribute('data-name') ? `${this.hasAttribute('data-name')}[${this.getAttribute('data-address-unknown')}]` : this.getAttribute('data-address-unknown')}"]`
      );
        ...
        if (addressUnknownCheckbox.checked) {
          Array.from(
            this.querySelectorAll(
              `input:not([name="${this.hasAttribute('data-name') ? `${this.getAttribute('data-name')}[${this.getAttribute('data-address-unknown')}]` : this.getAttribute('data-address-unknown')}"]), select`
            )
          ).forEach((input) => {
